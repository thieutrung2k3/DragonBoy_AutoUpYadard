...
{
	public MyVector findBoss = new MyVector();
	//public static Image logo = Image.createImage(File.ReadAllBytes("Dragon ball_240_Data\\Resources\\logo.png"));
	...
	public static void keoQuai()
	{
		
			if(global::Char.myCharz().cy != 456 && TileMap.mapID == 133)
			{
				GameScr.gI().checkClickMoveTo(190, 456, 0);
			}
			if (global::Char.myCharz().cx < 330 && global::Char.myCharz().cy == 456 && TileMap.mapID == 133)
			{
				int x = global::Char.myCharz().cx + 10;
                GameScr.gI().checkClickMoveTo(x, 456, 0);
            }
	}
	...
	private static void MoveMyChar(int x, int y)
	{
		...
		Service.gI().charMove();
		if (ItemTime.isExistMessage(4387))
		{
			return;
		}
		global::Char.myCharz().cx = x;
			...
			if (GameCanvas.keyAsciiPress != 0)
			{
				if (mobCapcha == null)
				{
					if (TField.isQwerty)
					{
						...
						else if(GameCanvas.keyAsciiPress == 106)
						{
							if (GameScr.getX(0) > 0 && GameScr.getY(0) > 0)
							{
						...
						{
							if(GameScr.getX(1) > 0 && GameScr.getY(1) > 0)
							{
                        ...
                        else if (GameCanvas.keyAsciiPress == 99)
                        {
							TzMod.useCapsule();
                        }
                        ...
                        else if (GameCanvas.keyAsciiPress == 102)
                        {
							TzMod.usePorata();
                        }
                        else if (GameCanvas.keyAsciiPress == 97)
                        {
							TzMod.isAutoYadard = !TzMod.isAutoYadard;
							new Thread(new ThreadStart(GameScr.keoQuai)).Start();
                        }
	...
	private void updateKeyTouchControl()
	{
		...
		for (int i = 0; i < saveChar.Length; i++)
		{
			if (saveChar[i] != null)
			{
				...
				if (GameCanvas.isPointerHoldIn(340, number, 150, 12))
				{
					GameScr.MoveMyChar(saveChar[i].cx, saveChar[i].cy);
					Char.myCharz().charFocus = saveChar[i];
	...
	{
		time_++;
		TzMod.onUpdateGameScr();
        if (GameCanvas.gameTick % 20 == 0)
        {
			TzMod.checkMapYadard();
        }
        if (time_ == 250)
        {
			keoQuai();
    ...
    public override void paint(mGraphics g)
	{
		...
		mFont.tahoma_7b_green.drawString(g, "[" + TileMap.zoneID + "]" + TileMap.mapName, 20, 50, 0);
		mFont.tahoma_7b_blue.drawString(g, DateTime.Now.ToShortTimeString(), 20, 60, 0);
		mFont.tahoma_7b_blue.drawString(g, "X: " + global::Char.myCharz().cx + " - Y: " + global::Char.myCharz().cy, 20, 70, 0);
        mFont.tahoma_7b_blue.drawString(g, "Auto bong tai: " + (TzMod.isAutoPorata ? "Bat" : "Tat"), 20, 80, 0);
        mFont.tahoma_7b_blue.drawString(g, "Auto change and ak: " + (TzMod.isChangeFocus ? "Bat" : "Tat"), 20, 90, 0);
        mFont.tahoma_7b_blue.drawString(g, "Auto keo quai: " + (TzMod.isAutoYadard ? "Bat" : "Tat"), 20, 100, 0);
        mFont.tahoma_7b_red.drawString(g, "So luong bi kip: " + TzMod.quantity, 20, 110, 0);
        //g.drawImage(GameScr.logo, 275, GameCanvas.h - 260, StaticObj.BOTTOM_HCENTER);
        resetTranslate(g);
		paint_xp_bar(g);
		if (!isPaintOther)
		{
	...
	public void chatVip(string chatVip)
	{
        ...
        {
			this.findBoss.addElement(new showBoss(chatVip));
			if(this.findBoss.size() > 5)
			{
				this.findBoss.removeElementAt(0);
			}
        }
